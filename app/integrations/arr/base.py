...
logger = logging.getLogger("guardarr.integrations.arr")
...
class BaseArrClient(ABC):
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> Optional[requests.Response]:
        ...
        try:
            ...
            logger.error(f"Arr API request error ({method} {endpoint}): {e}")
            return None
...
class SonarrClient(BaseArrClient):
    def __init__(self):
        ...
        super().__init__(
            ...
            verify_tls=settings.sonarr_verify_tls
        )
    ...
    def test_connectivity(self) -> Dict[str, Any]:
        ...
        return {"status": "disconnected", "reason": "Failed to connect or authenticate with Sonarr"}
...
class RadarrClient(BaseArrClient):
    def __init__(self):
        ...
        super().__init__(
            ...
            verify_tls=settings.radarr_verify_tls
        )
    ...
    def test_connectivity(self) -> Dict[str, Any]:
        ...
        if response and response.status_code == 200:
            try:
                ...
                return {"status": "connected", "version": "unknown"}
        return {"status": "disconnected", "reason": "Failed to connect or authenticate with Radarr"}


def get_arr_client(provider: str) -> BaseArrClient:
    if provider.lower() == "sonarr":
        return SonarrClient()
    elif provider.lower() == "radarr":
        return RadarrClient()
    else:
        raise ValueError(f"Unknown Arr provider: {provider}")
