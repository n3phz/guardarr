...
logger = logging.getLogger("guardarr.arr_orchestration")


class ArrOrchestrationService:
    @staticmethod
    def estimate(db: Session, req: ArrEstimateRequest) -> ArrEstimateResponse:
        if req.provider.lower() not in ["sonarr", "radarr"]:
    ...
    def admit(db: Session, req: ArrAdmitRequest) -> ArrAdmitResponse:
        ...
        idempotency_key = f"arr:{provider}:{req.content_id}:{req.arr_item_id or 'default'}"
        ...
        db.add(AuditLogModel(
            ...
            event_type="arr_admitted",
            details=f"Admitted via {provider} for content {req.content_id} (item={req.arr_item_id}) in RESERVED state"
        ))
        db.commit()
    ...
    def confirm_imported(db: Session, reservation_id: str, req: ArrImportedRequest) -> ArrImportedResponse:
        ...
        if req.content_id and reservation.content_id and req.content_id != reservation.content_id:
            db.add(AuditLogModel(
                reservation_id=reservation.id,
                event_type="ownership_verification_failed",
                details=f"Content ID mismatch: requested {req.content_id}, reservation has {reservation.content_id}"
            ))
            db.commit()
            raise ValueError("Content ID identity mismatch")

        if reservation.arr_item_id and req.arr_item_id and req.arr_item_id != reservation.arr_item_id:
            db.add(AuditLogModel(
                reservation_id=reservation.id,
                event_type="ownership_verification_failed",
                details=f"Arr item ID mismatch: requested {req.arr_item_id}, reservation has {reservation.arr_item_id}"
            ))
            db.commit()
            raise ValueError("Arr item ID identity mismatch")
        ...
        db.add(AuditLogModel(
            ...
            details=f"Explicit import ownership verified for {reservation.adapter_name} item {req.arr_item_id}. Transitioned ACTIVE -> OWNED."
        ))
        db.commit()
        db.refresh(reservation)
